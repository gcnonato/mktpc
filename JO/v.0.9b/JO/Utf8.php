...
class JO_Utf8 {
	...
	public static function mb_cut_text($str, $start, $length, $needle = false, $addon = false, $enc = 'UTF-8') {
		
		if(empty($str)) return '';
		
		if(mb_detect_encoding($str, $enc, true) === false) {
			$str = mb_convert_encoding(trim($str), $enc, mb_detect_encoding($str));
		}
		
		if(mb_strlen($str, $enc) > $length) {
			$str = mb_strcut($str, $start, $length, $enc);
			
			if($needle) {
				$pos = mb_strrpos($str, $needle, $enc);
				
				if($pos) {
					$str = mb_strcut($str, $start, $pos, $enc);
				}
			}
