...
class JO_View {
	...
	public function callChildren($children, $param = '') {
		...
		if($controller && $action) {
			...
			return $child->dispatch($controller, $action, $param);
		}
	...
	public function render($script, $controller) {
        ...
        //$this->childrens = new JO_View();
        
        if(is_array($this->children)) {
	        foreach($this->children AS $key => $child) { 
	        	$this->{$key} = $this->callChildren($child, $key);
//	        	$this->childrens->{$key} = $this->callChildren($child, $key);
//	        	if(is_callable($child)) {
//	        		$this->{$key} = call_user_func($child);
//	        	} else {
//	        		include_once 'JO/Exception.php';
//	        		$child = is_array($child) ? implode('->',$child) : $child;
//	        		throw new JO_Exception("Children " . $child . " is not callable");
//	        	}
	        }
        } 
